...
export async function updateProductCustomization(
  ...
  const errorMessage = "There was an error updating your banner";

  if (!success || userId == null || !canCustomize) {
    return { error: true, message: errorMessage };
  }

  const isSuccess = await updateProductCustomizationDb(data, { id, userId });

  return {
    error: !isSuccess,
    message: isSuccess ? "Banner updated" : errorMessage,
  };
}
...
export async function deleteProduct(id: string) {
  ...
  return {
    error: !isSuccess,
    message: !isSuccess ? errorMessage : "Successfully deleted your product",
  };
}

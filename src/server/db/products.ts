...
export function getProducts(
  ...
  const cacheFn = dbCache(getProductsInternal, {
    tags: [getUserTag(clerkUserId, CACHE_TAGS.products)],
  });
...
export async function createProduct(data: typeof ProductTable.$inferInsert) {
  ...
  try {
    ...
    // delete product if insertion in customization table is failed
...
export async function updateProductCustomization(
  ...
  const { rowCount } = await db
    ...
    .set(data)
    .where(eq(ProductCustomizationTable.productId, id));
...
function getProductsInternal(
  ...
  return db.query.ProductTable.findMany({
    where: ({ clerkUserId: clerkUserIdCol }, { eq }) =>
      eq(clerkUserIdCol, clerkUserId),
    orderBy: ({ clerkUserId }, { desc }) => desc(clerkUserId),
    limit,
  });
}
...
async function getProductCountryGroupsInternal({
  ...
  const data = await db.query.CountryGroupTable.findMany({
    with: {
      ...
      countries: {
        columns: {
          name: true, // country name
          code: true, // country flag
        },
      },
      // 对每个 product 是不一样的
  ...
  // 每个country group对应的countries 和它们对这个产品有的 discount and coupon
